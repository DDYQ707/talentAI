...
public class KnowledgeAdminController {
    ...
    @GetMapping("/{docId}")
    public R<KnowledgeDocVO> detail(
            @RequestHeader(value = "X-User-Role", required = false) String role,
            @PathVariable Long docId) {
        ChatAuthSupport.requireHr(role);
        try {
            return R.ok(knowledgeIngestService.getDocDetail(docId));
    ...
    @PutMapping("/{docId}")
    public R<KnowledgeDocVO> update(
            @RequestHeader(value = "X-User-Role", required = false) String role,
            @PathVariable Long docId,
            @RequestBody KnowledgeUpdateRequest request) {
        ChatAuthSupport.requireHr(role);
        try {
            return R.ok(knowledgeIngestService.updateDocument(docId, request));
        } catch (IllegalArgumentException e) {
            return R.fail(e.getMessage());
        }
    }
