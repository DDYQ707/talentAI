...
```
talent-ai-system/
├── docker/                  # Docker Compose（MySQL / Redis / Nacos / MinIO）
├── docs/
│   └── sql/                 # 按微服务拆分的建库建表脚本（见 README.md）
├── talent-ai-backend/       # Java 微服务（网关 + 5 个业务服务）
...
MySQL 有 **两种用法**，二选一即可（不要同时往两个库各建一套，容易混）：

| 方案 | 说明 | 后端 `application.yml` |
|------|------|------------------------|
| **A. 本机 MySQL（推荐，团队默认）** | 电脑已装 MySQL，端口 **3306** | **无需改**，仓库已配好 |
| **B. Docker MySQL** | 随 Compose 起的容器，宿主机端口 **3307** | 需把 JDBC 改为 `3307` + 密码 `root123` |

Redis、Nacos、MinIO **无论选哪种 MySQL，都建议用 Docker 启动**。

### 4.1 启动 Docker（Redis / Nacos / MinIO，MySQL 可选）
...
**若使用本机 MySQL（方案 A）**，可只起中间件、不起 Docker MySQL：
...
| 容器名 | 服务 | 宿主机端口 | 默认账号 | 方案 A 是否需要 |
|--------|------|------------|----------|-----------------|
| talent-mysql | MySQL 8.0 | **3307** → 容器内 3306 | root / root123 | 否（用本机 3306 时可停掉） |
| talent-redis | Redis 7 | **6380** → 6379 | 无密码 | 是 |
| talent-nacos | Nacos 2.3 | **8848**、9848、9849 | nacos / nacos | 是 |
| talent-minio | MinIO | **9000**、**9001** | minioadmin / minioadmin | 是 |

> **3307 与 3306 的关系**：Docker 把容器里的 3306 **映射**到宿主机 3307，避免与本机 MySQL 的 3306 冲突。后端连哪个端口，取决于你选方案 A 还是 B（见第七节）。  
> Docker 首次启动 MySQL 时，`docker/mysql/init.sql` 只会创建 6 个空库，**表结构仍需第六节 SQL**。
...
脚本位于 `docs/sql/`，每个文件含 **CREATE DATABASE + 建表**。  
**执行 SQL 时的端口/密码必须与你选的 MySQL 方案一致**（见第四节、第七节）。

详细说明见 [docs/sql/README.md](docs/sql/README.md)。

### 6.1 方案 A：本机 MySQL（3306，推荐）

仓库 `application.yml` 默认：`127.0.0.1:3306`，密码 `dyq!`（请改成你本机 root 密码，或把 MySQL 密码设为与 yml 一致）。
...
$mysqlArgs = @("-h127.0.0.1", "-P3306", "-uroot", "-p你的密码", "--default-character-set=utf8mb4")
$scripts = @(
...
foreach ($s in $scripts) {
  Write-Host ">>> $s"
  Get-Content $s -Raw -Encoding UTF8 | mysql @mysqlArgs
}
```
...
### 6.2 方案 B：Docker MySQL（3307）

仅在使用 Docker 里的 MySQL 时执行（端口 **3307**，密码 **root123**）：

```powershell
$mysqlArgs = @("-h127.0.0.1", "-P3307", "-uroot", "-proot123", "--default-character-set=utf8mb4")
# 其余同 6.1 的 $scripts 循环
```
...
将 `-P3306` / 密码换成你实际使用的（本机或 Docker）：

```powershell
Get-Content docs/sql/seed_admin_user.sql -Raw -Encoding UTF8 | mysql -h127.0.0.1 -P3306 -uroot -p你的密码 --default-character-set=utf8mb4
```

| 账号 | 密码 | 说明 |
|------|------|------|
| admin | 123456 | 管理员（user_type=4） |

演示候选人数据（需先在前端注册手机号 `13800138099`）：

```powershell
Get-Content docs/sql/seed_candidate_13800138099.sql -Raw -Encoding UTF8 | mysql -h127.0.0.1 -P3306 -uroot -p你的密码 --default-character-set=utf8mb4
```
...
| 数据库 | 微服务 | 端口 | SQL 脚本 |
|--------|--------|------|----------|
| talent_auth_db | talent-auth | 8081 | talent_auth_schema.sql |
| talent_job_db | talent-job | 8082 | talent_job_schema.sql |
| talent_resume_db | talent-resume | 8083 | talent_resume_schema.sql |
| talent_ai_db | talent-ai-agent | 8084 | talent_ai_agent_schema.sql |
| talent_interview_db | talent-interview | 8085 | talent_interview_schema.sql |
| talent_pool_db | talent-talent-pool | — | talent_pool_schema.sql（后端规划中） |
...
**原则：SQL 导入端口 = `application.yml` 里 JDBC 端口 = 实际在用的 MySQL 实例。**

### 方案 A：本机 MySQL（3306）— 团队默认，一般不用改 yml
...
3. `password` 与你在本机 MySQL 设置的 root 密码一致

`talent-auth` 的 Redis 仍连 Docker（6380），与 MySQL 方案无关：
...
### 方案 B：Docker MySQL（3307）

若不用本机 MySQL、改用 Docker 容器，需修改以下 5 个文件的 `url` 端口为 **3307**、`password` 为 **root123**：
...
spring:
  datasource:
    url: jdbc:mysql://127.0.0.1:3307/talent_xxx_db?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=Asia/Shanghai
    username: root
    password: root123
```

第六节 SQL 需用 **`-P3307 -proot123`** 导入。
...
# Docker 3307 能否登录
mysql -h127.0.0.1 -P3307 -uroot -proot123 -e "SHOW DATABASES LIKE 'talent_%';"
```

后端启动报错 `Communications link failure` 时，多半是 **yml 端口与正在用的 MySQL 不一致**。
...
| 顺序 | 模块 | 启动类 | 端口 |
|------|------|--------|------|
| 1 | talent-gateway | `GatewayApplication` | **8080** |
| 2 | talent-auth | `AuthApplication` | 8081 |
| 3 | talent-job | `TalentJobApplication` | 8082 |
| 4 | talent-resume | `TalentResumeApplication` | 8083 |
| 5 | talent-ai-agent | `AiAgentApplication` | 8084 |
| 6 | talent-interview | `TalentInterviewApplication` | 8085 |

> `talent-talent-pool`、`talent-analytics` 当前为骨架模块，**无需启动**。
...
打开 Nacos → **服务管理 → 服务列表** → 命名空间选 `talent-ai-dev`，应看到 6 个服务均为健康实例。
...
- [ ] `docker ps` 中 Redis / Nacos / MinIO 均为 Up（MySQL 容器可选）
- [ ] MySQL 6 个库、表已创建
- [ ] Nacos 命名空间 `talent-ai-dev` 已创建
- [ ] 6 个后端服务启动无报错
- [ ] Nacos 服务列表可见 6 个实例
- [ ] 前端 `npm run dev` 可访问
- [ ] 使用 admin / 123456 或自行注册账号可登录
...
1. 确认本机 MySQL（3306）或 Docker MySQL（3307）**至少有一个在跑且已建库建表**
2. 对照第七节：`application.yml` 的端口/密码必须与**实际导入 SQL 的那套**一致
3. 不要混用：在 3306 导入了 SQL，yml 却写 3307（或反过来）
...
3306   MySQL（本机安装，后端 application.yml 默认）
3307   MySQL（Docker 映射，可选）
